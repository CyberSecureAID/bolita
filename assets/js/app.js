...
async function pulsarConectar() {
  ...
  try {
    ...
    aviso(e.message === 'NO_WALLET'
      ? 'No se detectó ninguna wallet. Instala MetaMask o Trust Wallet.'
      : 'Conexión cancelada', true);
  }
}
...
function pintarMonedas() {
  ...
  const activo = S.paso >= 2;

  bloque.dataset.estado = activo ? (S.moneda ? 'hecho' : 'activo') : 'bloqueado';
  $('hint-moneda').textContent = activo
    ? 'Solo se admiten estas. Se marcan las que tienes.'
    : 'Conecta primero tu wallet';
...
function pintarModos() {
  ...
  const activo = S.paso >= 3 && S.moneda;

  bloque.dataset.estado = activo ? (S.modo ? 'hecho' : 'activo') : 'bloqueado';
...
function elegirModo(m) {
  if (S.paso < 3 || !S.moneda || S.girando) return;
  S.modo = m;
  S.seleccion = [];
  S.paso = 4;
  simularOcupado();
...
function pintarRejilla() {
  ...
  const activo = S.paso >= 4 && S.modo && S.moneda;
  bloque.dataset.estado = activo ? 'activo' : 'bloqueado';
...
function pintarBoleta() {
  ...
  if (!wallet.cuentaActual()) {
    btn.disabled = true; btn.textContent = 'Conecta tu wallet';
    $('i-jugadas').textContent = '0'; $('i-reparto').textContent = '—'; $('i-pago').textContent = '—';
    $('avisos').innerHTML = ''; return;
  }
  if (!S.moneda) { btn.disabled = true; btn.textContent = 'Elige una moneda'; return; }
...
function pintarCharada() {
  ...
  for (const it of CHARADA) {
    ...
    if (S.paso >= 4 && S.modo && S.modo.rango === 100) {
...
function pintarPasos() {
  $$('.paso').forEach((li) => {
    ...
    li.className = 'paso' + (n < S.paso ? ' hecho' : n === S.paso ? ' activo' : '');
  });
}
...
function init() {
  ...
  $('verso-n').textContent = `${v.numero}/${v.total}`;
