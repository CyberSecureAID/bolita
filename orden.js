...
/** Revisa el estado on-chain de cada orden limit guardada y limpia las que ya
    NO están pendientes (se llenaron al llegar el precio, o se cancelaron): el
    contrato pone `R.activa === false`. Así "Mis órdenes" no muestra órdenes
    fantasma ya cumplidas. Devuelve true si limpió alguna. */
export async function sincronizarOrdenes(cuenta) {
  const ords = leerOrdenesGuardadas();
  if (!cuenta || !ords.length) return false;
  let gb; try { gb = await import('./gridbot.js?v=125'); } catch (_) { return false; }
  let cambio = false;
  for (const o of ords) {
